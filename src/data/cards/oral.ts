import { deck } from './_deck';

export const oral = deck(
  'oral',
  [
    [2, 'Susúrrale al oído a {pareja} qué te gustaría hacerle con su verga, y acaríciasela de forma juguetona por encima de la ropa.', { targetGender: 'hombre' }],
    [2, 'Susúrrale al oído a {pareja} qué te gustaría hacerle con la lengua entre las piernas, y acaríciala por encima de la ropa mientras se lo dices.', { targetGender: 'mujer' }],
    [2, 'Pregúntale a {pareja} qué prefiere: dar o recibir sexo oral. Que te explique por qué con detalles.'],
    [2, 'Confiésale a {pareja} en qué momento del día de hoy pensaste en tenerlo en la boca.', { targetGender: 'hombre' }],
    [3, 'Métele un dedo en la boca a {pareja} y pregúntale qué le gustaría estar chupando en vez de tu dedo.'],
    [3, 'Pregúntale a {pareja} qué es lo que más le gusta de cómo se lo haces con la boca, y qué le cambiaría.'],
    [3, 'Pregúntale a {pareja} cuánto tiempo cree que aguantaría si te pones ahora mismo de rodillas. Que ponga un número.'],
    [4, 'Bésale la parte interna del muslo a {pareja} y pregúntale, sin subir todavía, cómo quiere que se lo haga: despacio o sin piedad.'],
    [4, 'Pregúntale a {pareja} dónde le gustaría que le hicieran sexo oral que nunca se han atrevido, y por qué no lo han hecho.'],
    [5, 'Cuéntale a {pareja} tu fantasía más sucia con la boca de alguno de los dos. Todos los detalles, sin saltarte nada.'],
  ],
  [
    [2, 'Chúpale los dedos a {pareja} uno por uno, mirándol{p} a los ojos.'],
    [2, 'Lámele el cuello y la clavícula a {pareja} durante treinta segundos.'],
    [3, 'Ponte de rodillas frente a {pareja} y bésale por encima de la ropa interior durante un minuto, sin quitársela.'],
    [3, 'Lámele el abdomen a {pareja} bajando hasta el borde de la ropa interior. Ahí te detienes y esperas.'],
    [3, 'Quítale la ropa interior a {pareja} con los dientes.'],
    [4, 'Hazle sexo oral a {pareja} durante un minuto exacto. Cuando se acabe el minuto, paras aunque te ruegue.'],
    [4, 'Chúpasela a {pareja} mientras te mira a los ojos, sin dejar de mirarle ni un segundo.', { targetGender: 'hombre' }],
    [4, 'Cómele el coño a {pareja} mientras te mira a los ojos, sin dejar de mirarle ni un segundo.', { targetGender: 'mujer' }],
    [4, 'Hazle sexo oral a {pareja} con las manos en la espalda. Solo boca y lengua.'],
    [4, 'Deja que {pareja} te agarre la cabeza y marque el ritmo mientras se la chupas.', { targetGender: 'hombre' }],
    [4, 'Deja que {pareja} te agarre la cabeza y marque el ritmo mientras se lo comes.', { targetGender: 'mujer' }],
    [5, 'Hazle sexo oral a {pareja} hasta que esté a punto de acabar, y ahí te detienes.', { tags: ['sexo.control_orgasmo'] }],
    [5, 'Los dos a la vez: sesenta y nueve durante dos minutos.'],
    [5, 'Métetela hasta el fondo de la garganta tres veces seguidas.', { targetGender: 'hombre', tags: ['oral.garganta'] }],
    [5, 'Hazle sexo oral a {pareja} hasta el final, sin parar, y termina en tu boca.', { tags: ['oral.acabar_boca'] }],
  ],
);
